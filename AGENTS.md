...
This workspace has **35+ specialized skills** you can load on-demand. Skills are detailed instructions for specific tasks.

### MANDATORY: Skills-First Protocol

Before starting ANY specialized task, you **MUST**:

1. **Read the index FIRST** (lightweight, <2KB):
   ```
   Read .claude/skills-index.json
   ```

2. **Scan descriptions** to find the relevant skill for your task

3. **Load ONLY that skill** by reading its SKILL.md:
   ```
...
If no skill exists for the task, proceed with general best practices.

### When Skills Apply (Examples)

| You should load a skill when... | Example skill |
|--------------------------------|---------------|
| Starting a code review | `code-review` |
| Creating issues or tasks | `task-design` |
| Performing git operations | `git`, `git-hygiene` |
| Writing documentation | `documentation-writing`, `api-documentation` |
| Setting up CI/CD | `ci-cd`, `deployment` |
| Doing a brainstorming session | `brainstorming` (uses `ask_questions` tool) |
| Creating a proof of concept | `proof-of-concept` |
| Refactoring code | `refactoring` |
| Writing tests | `tdd`, `testing-strategy` |
| Assimilating a repository | `orchestrator` (in assimilation) |

**This is not exhaustive.** Always check the index for the full list of 35+ skills.

### Quick Reference

- **Index (read first):** `.claude/skills-index.json`
- **Skills location:** `.claude/skills/{category}/{skill-name}/SKILL.md`
- **Categories:** core, engineering, devops, documentation, research, assimilation
...
For efficient, structured dialogue with users, prefer the `ask_questions` tool over plain text questions.

**Key Features:**
- Native UI for option selection
- Supports multiple choice, free text, or both
- Batch up to 4 related questions
- Automatic "Other" option added
...
```json
{
  "questions": [{
    ...
    "options": [
      ...
      {"label": "Option B", "recommended": true}
    ],
...
**When to use:**
- Clarifying user intent before implementation
- Choosing between design approaches
- Validating assumptions
- Brainstorming sessions (see `brainstorming` skill)

**When NOT to use:**
- Information determinable from code/context
- Reporting problems (attempt to resolve instead)
- Single trivial questions with obvious answers
...
| [task-design](/.claude/skills/core/task-design/) | Designing well-scoped tasks | Task specification |
...
| [code-simplifier](/.claude/skills/core/code-simplifier/) | Simplify complex code | Reducing complexity |
...
| [brainstorming](/.claude/skills/core/brainstorming/) | Idea-to-design dialogue | Design sessions |
| [agent-runbook](/.claude/skills/core/agent-runbook/) | Agent operation procedures | Agent workflows |
| [registry-manager](/.claude/skills/core/registry-manager/) | Skills registry management | Registry ops |
...
Located in `/.claude/skills/assimilation/`:

- **orchestrator** — Repository assimilation orchestration
- **pattern-extractor** — Extract patterns from existing code
- **repo-scanner** — Scan and analyze repositories
- **skill-generator** — Generate new skills from patterns
